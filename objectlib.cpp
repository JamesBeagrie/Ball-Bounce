...
void createPolygon(int sides, vec2 c, float radius, Object *obj) {
    for(int i = 0; i < sides; i++) {
        ...
        obj->e.push_back(Edge(p1,p2,d1,d2));
    }
}
...
void createWaveBowl(vec2 c, float width, float height, int segments, Object* obj) {
    ...
    for (int i = 0; i < segments; i++)
    {
        ...
        vec2 p1 = {x1, y1};
        vec2 p2 = {x2, y2};
        vec2 d1 = (p1 - c) / 1000.0f;
        vec2 d2 = (p2 - c) / 1000.0f;

        obj->e.push_back(Edge(p1,p2,d1,d2));
    }
}

void createCircleHell(vec2 c, float startingRadius, float gap, float holeAngle, int segments, std::vector<Object> environment) {
